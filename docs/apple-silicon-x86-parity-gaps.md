...
runtime bundle and before the package sync. It follows the signed package
channel (`asahi-packages-channel-<S>`, found through an R2 pointer), which
names the promoted `asahi-packages-stable-<commit>` release and the older sets
it supersedes; verifies that release's signed `CANDIDATE` descriptor against
the shipped `default/omarchy-arm-repository.asc` signing subkey and the tag's
own source commit; moves only from a superseded set; and then rewrites only
the `Server` line of the existing `[omarchy]` block after backing the file up
under `/var/lib/omarchy/backups/`. See
[the package channel](apple-silicon-distribution-channels.md#the-package-channel). The ISO pin above is still worth updating so
