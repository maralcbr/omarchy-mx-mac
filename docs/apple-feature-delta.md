...
| `bin/omarchy-update-asahi-legacy-repository` | Remove omarchy-mac's `[omarchy-aarch64]` from `pacman.conf` (via `install/hardware/pacman.sh`) and replace `obsidian-appimage` / `hyprland-preview-share-picker-git` with the `[omarchy]` packages; move its stale sync database out of pacman's sync directory, keeping a copy only while an installed package came from it. | Scott's configs still ship `[omarchy-aarch64]` | No | `mx runtime` (crossing from omarchy-mac) |
