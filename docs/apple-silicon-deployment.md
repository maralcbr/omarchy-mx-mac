...
image that already carries the package closure. It requires `--deferred-user`
(named-user provisioning is not an offline path). It does not call
`omarchy-update-asahi-repository --bootstrap`; `OMARCHY_ASAHI_KEEP_SERVER` must
name the expected `asahi-packages-stable-<sha>` Server, which `[omarchy]` must
already match. It runs the Aurora updater with `OMARCHY_AURORA_OFFLINE=1`, and
installs with `pacman -Su` (never `-Sy`/`-Syu`) after `pacman -Sup`, applying
that transaction to a disposable copy of the local database, then
`pacman --dbpath <copy> -Up --print` confirm every named archive is in
`/var/cache/pacman/pkg` with the database sha256. `LocalFileSigLevel` must
require package signatures (`PackageRequired`, or `Required` without
`PackageOptional`). Offline exits: **4** cache or printed-transaction
verification mismatch; **5** configuration or pin mismatch
(`OMARCHY_ASAHI_KEEP_SERVER`, `[omarchy]` Server, `LocalFileSigLevel`); **6**
Aurora updater refused the offline pin or descriptor.
...
2. **VM acceptance** on a test Mac with `/dev/kvm` (the M2 Max):
   ...
   `~/vm-evidence/<run-id>/`. The VM boots a generic kernel, so it does not
   qualify Aurora, m1n1 or the Apple boot chain.
3. **Stage the release** (this Mac). Download the release assets, reassemble
   the zip and check it against `PROVENANCE`. Write `inputs.json`: payload name,
   engine fields (`engine_name`, `engine_version`, `asahi_installer_*`,
   `downstream_revision`), `device_identifiers`, `evidence_revision` and the
...
4. **Owner signs** `catalog/catalog.json` with `catalog-signing.swift
...
5. **Publish and promote.** `scripts/publish-m1-release publish-r2 --dir staged
   ...
   qualified, `--to stable --no-prune`. `stable` without `--no-prune` deletes the
   older releases.
6. **Tag** `os-v<version>.<date>-rc` on the mx commit the installer was built
   from, and name the Omarchy version the image carries: this image's tag says
   4.0.3 but it carries the 4.0.4 runtime.
...
runs it. `rc` and `edge` carry them. A display-count check on a Mac running
...
- Leftover `linux-asahi-headers` after an Asahi-to-Aurora move owns a newer
  ...
  successful non-Aurora record also settles. A user-set DTBS is not this hazard, so completion skips
