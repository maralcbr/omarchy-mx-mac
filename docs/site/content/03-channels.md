...
2. **Legacy repository cleanup.** On a Mac that came from the omarchy-mac project, `omarchy-update-asahi-legacy-repository` removes its unsigned `[omarchy-aarch64]` repository from `/etc/pacman.conf` (backup under `/var/lib/omarchy/backups/`), keeps `[omarchy]` ahead of what that repository shadowed, and replaces `obsidian-appimage` and `hyprland-preview-share-picker-git` with the `[omarchy]` packages. It also replaces those two packages where they came from elsewhere, such as the AUR. Once no pacman configuration file lists the repository, it takes that repository's leftover sync files out of `/var/lib/pacman/sync/`. Signatures and file lists are deleted. The database moves to `/var/lib/omarchy/retired-repositories/`, where the bundle step still reads it, while an installed package still has a build from it (same name, version and build date), and is deleted once none does. When that cannot be decided (an unreadable database or local package entry), or an earlier kept copy that differs still proves a package, the database stays. On a Mac with none of the repository, those packages or those files it does nothing. A step that cannot finish keeps the section, the legacy package or the database in place and the next update tries again; the update carries on either way.
