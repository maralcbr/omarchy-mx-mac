...
3. Pick a channel. The app offers `rc`, with the Aurora kernel and its external-display support, by default; `stable`, with the Asahi kernel, is in the **Release Channel** menu. Choose carefully: the kernel family is fixed at install. See [Channels and updates]({{page:channels}}).
