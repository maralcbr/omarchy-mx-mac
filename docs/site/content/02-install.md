...
3. Pick a channel. `stable` is the default for new users; `rc` carries the Aurora kernel with better external-display support. See [Channels and updates](../channels/).
...
The Mac reboots into Omarchy. The [first boot](../install-flow/) installs vendor firmware, converts the root to LUKS if you asked for it, creates your user and lands on the desktop.
