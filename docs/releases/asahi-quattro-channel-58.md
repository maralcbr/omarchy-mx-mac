...
- `omarchy-settings-dev` gives Apple Silicon the Omarchy bash skeleton
  (omarchy-pkgs #201).
...
- **Dictation.** Macs install the prebuilt `voxtype-bin` instead of a 10-20
  minute AUR build (#245, refs #114).
- **Fresh install.** A resume that cannot continue names each field that
  differs and says how to resume (#246, refs #85).
- **Boot check.** On an encrypted Mac with a non-US Latin layout, the boot
  check verifies that the image the Mac boots carries the current keyboard
  layout (#247, refs #235).
...
- **Hardware** (boot packages changed): the M2 Max with a LUKS2 root and
  ...
  the desktop came up, the boot check passed, no units failed, and the speakers
  worked. Record:
