...
! grep -Fq 'systemd/zram-generator\.conf\.d' "$upgrade_to_quattro" || fail "Omarchy 4 upgrade accepts the zram drop-in on Apple Silicon"
! grep -Fq 'omarchy-zswap\.conf' "$upgrade_to_quattro" || fail "Omarchy 4 upgrade accepts the zswap tmpfile on Apple Silicon"
