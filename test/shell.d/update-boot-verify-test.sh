...
# Apple: preflight runs before the keyring and packages change, and verify once
# the last package step is done, before hooks, mise and the reboot offer.
...
dev=$(at 'step:omarchy-update-dev') preflight=$(at 'sudo -N omarchy-lifecycle-dispatch update-preflight')
...
[[ -n $dev && -n $preflight && -n $keyring && -n $aur && -n $verify && -n $hook ]] &&
  (( dev < preflight && preflight < keyring && aur < verify && verify < hook )) ||
  fail "apple: preflight follows the dev checkout and precedes the keyring; verify follows AUR and precedes the hook" "$(cat "$SUDO_TEST_LOG")"
[[ $(sed -n "$((verify + 1))p" "$SUDO_TEST_LOG") == "sudo -k" ]] || fail "apple: sudo is revoked right after verification" "$(cat "$SUDO_TEST_LOG")"
...
grep -q '^step:omarchy-hook post-update' "$SUDO_TEST_LOG" && grep -q '^step:omarchy-update-stay-awake stop' "$SUDO_TEST_LOG" ||
  fail "apple: after a failed verification the hooks still run and Stay Awake is released" "$(cat "$SUDO_TEST_LOG")"
