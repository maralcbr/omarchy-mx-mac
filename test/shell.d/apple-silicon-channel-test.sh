...
lock="$root/run/lock/omarchy-apple-silicon-channel.lock"
calls="$test_tmp/calls"
pacman_calls="$test_tmp/pacman-calls"
mkdir -p "$stub_bin" "$state_dir" "$(dirname "$marker")" "$root/run/lock"
...
run() {
  ...
  TEST_CALLS="$calls" \
    ...
    timeout "${TEST_RUN_TIMEOUT:-30}" bash "$helper" "$@" >"$test_tmp/out" 2>"$test_tmp/err"
  status=$?
  set -e
}
...
grep -Fxq "sudo:install -m 0644 -o root -g root /dev/null $lock" "$calls" || fail "the machine lock is created root-owned" "$(cat "$calls")"
...
[[ ! -s $test_tmp/out && ! -s $test_tmp/err && ! -s $calls && ! -e $record && ! -e $lock ]] ||
...
[[ ! -s $calls && ! -e $lock && ! -e $record ]] || fail "status and current take no lock and write nothing" "$(cat "$calls")"
reset
write_record 'format=1\nchannel=edge\nkernel=linux-aurora\n'
run status
...
grep -Fq "could not lock $lock within 0.3s" "$test_tmp/err" || fail "a lock timeout is explained" "$(cat "$test_tmp/err")"
