...
forbidden='^(linux|linux-headers|linux-ptl|linux-ptl-headers|linux-omarchy|linux-omarchy-headers|broadcom-wl(-dkms)?|egl-wayland|intel-.*|libva-intel-driver|libva-nvidia-driver|libvpl|vpl-gpu-rt|sof-firmware|thermald|nvidia-.*|lib32-.*|macbook12-spi-driver-dkms|apple-bcm-firmware|apple-t2-audio-config|linux-t2|linux-t2-headers|t2fanrd|tiny-dfr|tuxedo-drivers-nocompatcheck-dkms|yt6801-dkms|kvantum-qt5|qt5-wayland|rust)$'
