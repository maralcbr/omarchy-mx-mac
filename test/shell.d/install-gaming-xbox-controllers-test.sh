...
cat >"$stub_bin/omarchy-hw-apple-kernel" <<'STUB'
#!/bin/bash
echo "${TEST_KERNEL:-linux-asahi}"
STUB
chmod +x "$stub_bin"/*
...
TEST_KERNEL=linux-aurora APPLE_SILICON=1 run_install >"$test_tmp/aurora-out"
grep -qx 'omarchy-pkg-add linux-aurora-headers xpadneo-dkms' "$calls" || fail "Aurora installer headers"
resolved=$(TEST_KERNEL=linux-aurora APPLE_SILICON=1 OMARCHY_PATH="$ROOT" PATH="$stub_bin:$PATH" bash -c '
  source "$OMARCHY_PATH/install/helpers/optional-packages.sh"
  __omarchy_optional_targets install.gaming.xbox-controllers
  printf "%s " "${__omarchy_requested_packages[@]}"
')
[[ $resolved == 'linux-aurora-headers xpadneo-dkms ' ]] || fail "Aurora availability headers" "$resolved"
pass "Aurora installer and availability select matching headers"
