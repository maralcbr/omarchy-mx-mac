...
# omarchy-drive-password against fake drives on Apple Silicon unless a case says
# otherwise. blkid, findmnt and lsblk describe a system drive under / and a data
# drive; cryptsetup is either a slot-table fake
...
fixture() {
  ...
  unset TEST_OPEN_FAIL TEST_CHPASSWD_FAIL TEST_CHANGE_FAIL TEST_CHANGE_PARTIAL TEST_KILL_FAIL TEST_FINDMNT_FAIL TEST_TOKEN_SLOT CRASH_ORPHAN TEST_APPLE
...
fixture
export TEST_APPLE=0
attempt 0 "$old_password" "$new_password" "$new_password" || fail "a system disk off Apple Silicon changes" "$(cat "$tmp/output")"
[[ -n $(opens "$system" "$new_password") && $(account owner) == "$old_password" && $(account root) == "$old_password" ]] ||
  fail "off Apple Silicon the system disk changes alone"
! grep -q '^chpasswd' "$tmp/sudo-calls" && [[ ! -e $journal ]] || fail "off Apple Silicon no chpasswd runs and no journal is kept"
unset TEST_APPLE
pass "off Apple Silicon the drive password changes the LUKS key only"
