...
grep -Fq 'omarchy-provision-user "${finalize_user_args[@]}" || finalize_user_status=$?' "$ROOT/bin/omarchy-provision-first-run" || \
  fail "first-run records user finalization failures"
grep -Fq 'first_run_failed=1' "$ROOT/bin/omarchy-provision-first-run" || fail "first-run keeps failed setup retryable"
grep -Fq 'exit 1' "$ROOT/bin/omarchy-provision-first-run" || fail "failed first-run exits non-zero"
