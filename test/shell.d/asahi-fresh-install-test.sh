...
grep -Fq '"$kernel_package" | "$kernel_package-headers" | "$m1n1_package" |' "$installer" || fail "fresh installer omits the installed kernel from explicit targets"
grep -Fq 'linux-aurora | linux-aurora-headers | m1n1-aurora | linux-asahi | linux-asahi-headers | m1n1)' "$installer" ||
  fail "fresh installer omits both kernel families, their headers and m1n1 from explicit targets, whichever kernel the image booted"
...
grep -Fq 'sha256sum --check --status <<<"$grub_sha256" || verify_grub_config' "$installer" ||
  fail "fresh installer resumes on the recorded GRUB configuration or one that boots the kernel"
# Once in verify_boot_files (the deferred path), once at the end of a normal install.
(( $(grep -c '^  verify_grub_config$' "$installer") == 2 )) || fail "fresh installer verifies that GRUB boots the kernel at the end"
